 ...
 * one (Postgres rejects it). A plain `CREATE INDEX` takes an ACCESS EXCLUSIVE
 * lock on the target table for the duration of the build, blocking writes to
 * it until it completes. At current data volume (dev/early-prod, low order
