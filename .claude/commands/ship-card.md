...
3. IMPLEMENT (dispatch specialists, in dependency order — work in a loop, one slice at a time)
   Both the Obsidian vault and this project are git repos shared via git. For each slice of
   ...
   - DOCUMENT: dispatch docs-writer to (a) append Implementation Notes for this slice to the
     relevant Obsidian spec note, and (b) record what happened this session in the vault's
     History folder. Session logs live at `History/<Name>/session-<n>.md`, where `<Name>` is
     the operator — `Josh` or `Michael`, matched to the local git user — and `<n>` increments
     from the highest existing `session-*.md` in that folder. Capture: card worked, what
     changed, key decisions, test/review outcome, and any follow-ups.
